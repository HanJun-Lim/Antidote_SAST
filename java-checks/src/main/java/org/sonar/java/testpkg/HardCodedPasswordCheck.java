...
public class HardCodedPasswordCheck extends IssuableSubscriptionVisitor {
	...
	private static final MethodMatcher PASSWORD_AUTHENTICATION_CONSTRUCTOR = MethodMatcher.create()
			.typeDefinition("java.net.PasswordAuthentication").name("<init>").addParameter(JAVA_LANG_STRING)
			.addParameter("char[]");
	...
	private static final MethodMatcher EQUALS_MATCHER = MethodMatcher.create().name("equals")
			.parameters(JAVA_LANG_OBJECT);

	private static final MethodMatcher GET_CONNECTION_MATCHER = MethodMatcher.create()
			.typeDefinition("java.sql.DriverManager").name("getConnection").withAnyParameters();
	...
	public List<Kind> nodesToVisit() {
		/*
		 * types to check example
		 * 
		 * LITERAL 123, "hello", 'a' VARIABLE num, var, etc.. ASSIGNMENT +=, -=, ^=, %=
		 * NEW_CLASS new Hello() METHOD_INVOCATION println("hello")
		 */

		return Arrays.asList(Tree.Kind.STRING_LITERAL, Tree.Kind.VARIABLE, Tree.Kind.ASSIGNMENT, Tree.Kind.NEW_CLASS,
	...
	private Optional<String> isSettingPassword(MethodInvocationTree tree) {
		...
		if (arguments.size() == 2 && isArgumentsSuperTypeOfString(arguments) && !isPasswordLikeName(arguments.get(1))
				&& isNotExcludedString(arguments.get(1))) {
	...
	private Optional<String> isPassword(ExpressionTree argument) {
		...
		return variablePatterns().map(pattern -> pattern.matcher(value))
				// should exactly match "pwd" or similar
				.filter(Matcher::matches).map(matcher -> matcher.group(1)).findAny();
	}
	...
	private void handleStringLiteral(LiteralTree tree) {
		...
		if (isURLWithCredentials(cleanedLiteral)) { // if the URL contains sensitive information..
			reportIssue(tree, "해당 하드 코딩된 URL은 패스워드 정보를 담고 있을 수 있으므로 재검토가 필요합니다");
			...
			literalPatterns().map(pattern -> pattern.matcher(cleanedLiteral))
					...
					.filter(match -> !isExcludedLiteral(cleanedLiteral, match)).findAny()
					.ifPresent(credential -> report(tree, credential));
		}
	}

	private static boolean isURLWithCredentials(String stringLiteral) {
		if (URL_PREFIX.matcher(stringLiteral).find()) { // if non-compliance detected..
			try { // valid URL only can execute the code below
				String userInfo = new URL(stringLiteral).getUserInfo();
				if (userInfo != null) { // if the user information detected..
	...
	private boolean isPartOfConstantPasswordDeclaration(LiteralTree tree) {
		...
		return parent != null && parent.is(Kind.VARIABLE)
				&& isPasswordVariableName(((VariableTree) parent).simpleName()).isPresent();
	}
	...
	private static boolean isExcludedLiteral(String cleanedLiteral, String match) {
		...
		return !isNotExcludedString(followingString) || followingString.startsWith("?")
				|| followingString.startsWith(":") || followingString.startsWith("\\\"")
	...
	private static boolean isArgumentsSuperTypeOfString(List<ExpressionTree> arguments) {
		return arguments.stream()
				.allMatch(arg -> arg.symbolType().is(JAVA_LANG_STRING) || arg.symbolType().is(JAVA_LANG_OBJECT));
	}
	...
	private void handleConstructor(NewClassTree tree) {
		...
		if (secondArg.is(Tree.Kind.METHOD_INVOCATION)) {
			...
			if (STRING_TO_CHAR_ARRAY.matches(mit) && isCallOnStringLiteral(mit.methodSelect())) {
				reportIssue(tree, "해당 하드 코딩된 패스워드를 제거하십시오");
			}
		}
	}
	...
	private void handleMethodInvocation(MethodInvocationTree mit) {
		...
		if (EQUALS_MATCHER.matches(mit) && methodSelect.is(Kind.MEMBER_SELECT)) {
			...
			isSettingPassword(mit)
					.ifPresent(settingPassword -> report(ExpressionUtils.methodName(mit), settingPassword));
		}
	}
	...
	private void handleEqualsMethod(MethodInvocationTree mit, MemberSelectExpressionTree methodSelect) {
		...
		isPasswordVariable(leftExpression).filter(
				passwordVariableName -> isNotExcludedString(rightExpression) && !isPasswordLikeName(rightExpression))
				.ifPresent(passwordVariableName -> report(leftExpression, passwordVariableName));

		isPasswordVariable(rightExpression).filter(
				passwordVariableName -> isNotExcludedString(leftExpression) && !isPasswordLikeName(leftExpression))
	...
	private void handleGetConnectionMethod(MethodInvocationTree mit) {
		if (mit.arguments().size() > GET_CONNECTION_PASSWORD_ARGUMENT) {
			...
			if (isNotExcludedString(expression)) {
				reportIssue(expression, "해당 하드 코딩된 패스워드를 제거하십시오");
			}
		}
	}

	private void report(Tree tree, String match) {
		reportIssue(tree, "'" + match + "' : 패스워드 정보가 담긴 것으로 의심됩니다, 해당 부분을 재검토해 주십시오");
	}
}
