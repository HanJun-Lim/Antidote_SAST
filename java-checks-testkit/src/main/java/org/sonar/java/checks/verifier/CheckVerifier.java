...
public abstract class CheckVerifier {
  ...
  public List<ErrorInfo> checkIssues(Set<AnalyzerMessage> issues, boolean bypassNoIssue) {
    ...
    if (expectNoIssues) {
      ...
      // --------------- issues 값 추출 ---------------
      Iterator<AnalyzerMessage> it = issues.iterator();
      AnalyzerMessage am;
      AnalyzerMessage.TextSpan ts;

      while(it.hasNext()) {
       // 오류 리스트에 항목 추가(ADD)
        am = it.next();
        ts = am.primaryLocation();
        
       ErrorInfo tmp = new ErrorInfo(am, ts);
        errorInfoList.add(tmp);
        
        // for debug
        System.out.println("***************** test message **************************");
        System.out.println("AnalyzerMessage toString() : " + am.toString());
        System.out.println("TextSpan toString() : " + ts.toString());
      }
      System.out.println("ErrorInfo List size : " + errorInfoList.size());
